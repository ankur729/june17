...
router
    ...
    .post('/fetch-associated-cooks', function (req, res, next) {
        ...
        db.service_center_infos.aggregate({ $match: { _id: ObjectId(req.body.service_center_id) } },
            ...
            { $project: { 'cook_data._id': 1, 'cook_data.cook_name': 1, 'cook_data.cook_email': 1, 'cook_data.cook_contact': 1, 'cook_data.cook_commission': 1, 'cook_data.joined_on': 1, 'cook_data.status': 1 } }
            //  { $unwind: "$cook_data" },
            //     {
            //     $lookup: {

            //         from: 'order_infos',
            //         localField: 'cook_data._id',
            //         foreignField: 'cook_id',
            //         as: 'total_order'
...
router
    .get('/fetch-user-orders-all', function (req, res, next) {
        ...
        db.order_infos.find({
                ...
                order_id: 1, date: 1, 'order_status': 1, 'items.grand_total': 1, 'items.username': 1
            },
            function (err, orders) {
                if (err || !orders) console.log("No  data found");
                else {
                    // console.log(order);
...
router
    .post('/fetch-delivery-boy-orders-by-id', function (req, res, next) {
        ...
        db.delivery_boy_infos.aggregate(
            ...
            ,
            // { $unwind: "$service_center.cook_arr" },
            // //    { $project: { 'delivery_boy._id': 1, 'delivery_boy.boy_name': 1, 'delivery_boy.boy_name': 1 } },
            // {
            //     $lookup: {
            //         from: 'order_infos',
            //         localField: 'service_center.cook_arr._id',
            //         foreignField: 'cook_id',
            //         as: 'orders'
...
router
    .post('/fetch-cook-orders-for-front', function (req, res, next) {
        ...
        db.order_infos.find({
            ...
            cook_id: mongojs.ObjectId('593ba5b7a8b58b06478ec3cd')
        },
        ...
        db.cook_infos.find({
                ...
                if (err || !cooks) console.log("No  cook found");
                    ...
                    db.order_infos.find({
                        ...
                        'items.sub_order_status': 'delivered'
                        ...
                        function (err, data) {
                            ...
                            var tot_amt = 0;
                            var tot_orders = 0;
                            var is_cook_found = false;

                            for (var k = 0; k < cook_info.length; k++) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    for (var j = 0; j < data[i].items.length; j++) {
                                        ...
                                        if (data[i].items[j].cook_id == cook_info[k]._id) {

                                            if (data[i].items[j].sub_order_status == 'delivered') {

                                                tot_amt = tot_amt + parseInt(data[i].items[j].food_total_price);
                                                tot_orders++;
                                                is_cook_found = true;
                                ...
                                if (is_cook_found == true) {

                                    obj = {};
                                    obj.cook_id = cook_info[k]._id;
                                    obj.cook_name = cook_info[k].cook_name;
                                    obj.cook_commission = cook_info[k].cook_commission;
                                    obj.total_amount_payble = tot_amt;
                                    obj.tot_orders = tot_orders;
                                ...
                                function (err, commission) {
                                    ...
                                    for (var m = 0; m < final_data.length; m++) {
                                        ...
                                        for (var n = 0; n < commission.length; n++) {
                                            ...
                                            if (final_data[m].cook_id == commission[n].cook_id) {

                                                tot_paid = tot_paid + commission[n].paid_amt;
                                                is_paid = true;
                                            }
                                        }
                                        ...
                                        if (is_paid == true) {
                                            ...
                                            final_data[m].total_paid = tot_paid

                                        }
                                        else {

                                            final_data[m].total_paid = 0;
                                        }
                                    }
                            ...
                            // if(data[i].items[j].cook_id==cook_info[k].cook_id){

                            //         obj={};
                            //         obj.cook_id=cook_info[k].cook_id;
                            //         obj.cook_name=cook_info[k].cook_name;
                            //         obj.cook_commission=cook_info[k].cook_commission;
